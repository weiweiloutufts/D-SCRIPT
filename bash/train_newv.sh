...
EMBEDDING=/hpc/home/wl324/projects/tt3d/data_archive/esm2/bernett
EMBEDDING_DIM=1280

OUTPUT_BASE=/hpc/home/wl324/projects/tt3d/data/results
OUTPUT_FOLDER=${OUTPUT_BASE}/bernett_esm2_train_newv_lr${lr}_lam${lam}_wd${wd}_dp${dp}_bk${bk}
...
usage() {
    ...
    -f: Disabled in this script version
    -F: Disabled in this script version
...
while getopts "d:t:fF:T:e:E:vo:p:" args; do
    case $args in
        ...
        v) TOPSY_TURVY="--topsy-turvy --glider-weight 0.2 --glider-thresh 0.925" ;;
...
$PY -m dscript.commands.train_newv_nomix \
    ...
    --save-prefix "${OUTPUT_FOLDER}/${OUTPUT_PREFIX}_newv" \
    --device "${DEVICE}" \
    --lr "${lr}" \
    --lambda "${lam}" \
    --num-epochs 20 \

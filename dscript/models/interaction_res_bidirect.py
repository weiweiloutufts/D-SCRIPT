...
class PairClassifierTokens(nn.Module):
    ...
    def _extract_map_summary(self, map_input: torch.Tensor, g_fused: torch.Tensor):
        ...
        g_map = self.map_g_proj(g_fused)[:, :, None, None].expand(
            B, self.map_global_channels, g, g
        )
        g_scale = 0.1
        x_pool = torch.cat([x_pool, g_scale * g_map], dim=1)        # [B, 1+K, g, g]
...
class ModelInteraction(nn.Module):
    def __init__(
        ...
        if contact_max_length < 1:
            raise ValueError("contact_max_length must be at least 1")
        self.contact_max_length = int(contact_max_length)
        if do_sigmoid:
    ...
    # don't have access to the A100 anymore, so compress the contact map
    def _compress_contact_embedding(self, embedding):
        target_length = min(embedding.shape[1], self.contact_max_length)
    ...
    def cpred(self, inputs):
        ...
        contact_e0 = self._compress_contact_embedding(e0)
        contact_e1 = self._compress_contact_embedding(e1)
        C = self.contact.predict(self.contact.cmap(contact_e0, contact_e1))
        ###added augment

"""
Train an interaction_res model with an order-consistency objective.
"""
...
from ..models.interaction_res_bidirect import InteractionInputs, ModelInteraction
...
def add_args(parser):
    ...
    inter_grp.add_argument(
        ...
        default=512,
        help=(
            "maximum residue bins per protein in the contact-map branch; "
            "longer embeddings are adaptively averaged (default: 512)"
        ),
    )
    ...
    train_grp.add_argument(
        ...
        type=float,
        default=1.0,
        help="weight for MSE consistency between predictions for (protein1, protein2) and (protein2, protein1) (default: 1.0)",
    )
    # Topsy-Turvy
    train_grp.add_argument(
        "--topsy-turvy",
...
## feeding protein pairs both ways
def predict_interaction(model, n0, n1, tensors, use_cuda, structural_context=None):
    logits12, _, _ = predict_cmap_interaction(
        model, n0, n1, tensors, use_cuda, structural_context
    )
    logits21, _, _ = predict_cmap_interaction(
        model, n1, n0, tensors, use_cuda, structural_context
    )
    return 0.5 * (logits12 + logits21)
...
def interaction_grad(
    ...
    pred12 = logits12.view(-1).float()
    pred21 = logits21.view(-1).float()
    logits_used = 0.5 * (pred12 + pred21)
    cls_loss = 0.5 * (
        F.binary_cross_entropy_with_logits(pred12, y)
        + F.binary_cross_entropy_with_logits(pred21, y)
    )
    if run_tt:
        ...
        glider_loss = 0.5 * (
            F.binary_cross_entropy_with_logits(pred12, g_score)
            + F.binary_cross_entropy_with_logits(pred21, g_score)
        )
        cls_loss = glider_weight * glider_loss + (1.0 - glider_weight) * cls_loss
    
    # Symmetry Loss 
    sym_loss = F.mse_loss(pred12, pred21)
    # Total Loss
    loss = cls_loss + symmetry_lambda * sym_loss
    ...
    loss_terms = {
        ...
        "sym_loss": float(sym_loss.detach().item()),
    }
...
def train_model(args, output):
    ...
    # Validate embeddings and load on demand to avoid preloading the full corpus.
    if allow_prostt5:
    ...
    if allow_prostt5:
        # Using ProstT5 as input (512 dim by default)
        ...
        # Using ESM2 or other embeddings as input (1280 dim by default)
    ...
    contact_max_length = args.contact_max_length
    ...
    log(f"\tcontact_max_length: {contact_max_length}", file=output)
    ...
    log(f"\tsymmetry_lambda: {args.symmetry_lambda}", file=output)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (z0, z1, y) in enumerate(train_iterator):
            ...
            sym_accum += b * (loss_terms["sym_loss"] - sym_accum) / n

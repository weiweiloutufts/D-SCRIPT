...
from ..models.interaction_newv import InteractionInputs, ModelInteraction
...
class TrainArguments(NamedTuple):
    ...
    lr: float
    lam_ent: float
    entropy_start_epoch: int
    ph_weight_max: float
    disable_ph_loss: bool
...
def add_args(parser):
    ...
    train_grp.add_argument(
        ...
        type=float,
        default=0.02,
        help="weight on token-entropy regularization (default: 0.02)",
    )
    train_grp.add_argument(
        "--entropy-start-epoch",
        type=int,
        default=0,
        help="epoch index at which to start entropy regularization (default: 0)",
    )
    train_grp.add_argument(
        "--ph-weight-max",
        type=float,
        default=0.05,
        help="maximum weight for the ph auxiliary loss; decays to 0 over the anneal window (default: 0.05)",
    )
    train_grp.add_argument(
        "--disable-ph-loss",
        action="store_true",
        help="disable the ph auxiliary loss entirely for ablation",
    )
    train_grp.add_argument(
        "--lambda",
...
def predict_cmap_interaction(
    ...
    def _batch_auxiliary_outputs(aux_list):
        if not aux_list:
            return {}

        aux_batched = {}
        expected_keys = set(aux_list[0])
        for aux in aux_list[1:]:
            if set(aux) != expected_keys:
                raise ValueError("Model auxiliary outputs must have the same keys per item")

        for k, v0 in aux_list[0].items():
            values = [a[k] for a in aux_list]
            if torch.is_tensor(v0):
                # Scalar tensors need stacking; batched tensors already carry a leading batch dim.
                aux_batched[k] = (
                    torch.stack(values, dim=0) if v0.ndim == 0 else torch.cat(values, dim=0)
                )
            elif isinstance(v0, (int, float, bool, str)) or v0 is None:
                aux_batched[k] = v0 if all(v == v0 for v in values[1:]) else values
            else:
                aux_batched[k] = values
    ...
    with ctx:
        for i in range(b):
            ...
            cm, logit, feat, aux = model.map_predict(
                InteractionInputs(
                    z_a, z_b,
            ...
            # store
            logits_list.append(logit.reshape(-1))              # [1]
            feat_list.append(feat)                           # [1, 2D]
            c_map_mag_list.append(cm.mean().reshape(1))        # [1]
            aux_list.append(aux)       

        logits = torch.cat(logits_list, dim=0)                  # [B]
        feat = torch.cat(feat_list, dim=0)                      # [B, F]
        c_map_mag = torch.cat(c_map_mag_list, dim=0)            # [B]

        aux_batched = _batch_auxiliary_outputs(aux_list)

    return c_map_mag, logits, feat, aux_batched


def predict_interaction(model, n0, n1, tensors, use_cuda, structural_context=None):
    _, logits, _, _= predict_cmap_interaction(model, n0, n1, tensors, use_cuda, structural_context)
    return logits
...
def entropy_floor_loss(token_w, target_H=2.0, eps=1e-8):
    w = token_w.clamp_min(eps)
    H = -(w * w.log()).sum(dim=1)  # [B]
    return torch.relu(target_H - H).mean()


def linear_anneal(step: int, warm_steps: int, start: float, end: float) -> float:
    """Linearly anneal from start -> end over warm_steps."""
    if warm_steps <= 0:
        return end
    frac = min(1.0, max(0.0, step / float(warm_steps)))
...
def interaction_grad(
    ...
    ph_weight_max=0.05,
    disable_ph_loss=False,
    lam_feat=5e-5,
    epoch=0,
):
    c_map_mag, logits, feat, aux = predict_cmap_interaction(
        model, n0, n1, tensors, use_cuda, structural_context, require_grad=True
    )

    b = len(n0)
    y = y.cuda() if use_cuda else y
    y = y.float().view(-1).to(logits.device)  # [B]

    # -----------------------------
    # Train on original features
    # -----------------------------
    logits_used, _ = model.clf.classify_from_feat(feat, aux["x_pool"])
    y_used = y
    ...
    if run_tt:
        ...
        g_score = torch.stack(g_score, 0).to(logits_used.device).view(-1)
        glider_loss = F.binary_cross_entropy_with_logits(logits_used.view(-1), g_score)
        bce_loss = glider_weight * glider_loss + (1.0 - glider_weight) * bce_loss

    loss = bce_loss
    ent_unweighted = torch.zeros((), device=logits.device)

    # -----------------------------
    # ph loss
    # -----------------------------

    ph_loss = F.binary_cross_entropy_with_logits(
        aux["ph_logit"].view(-1),  # flatten to [B]
        y.view(-1),
    )
    ph_weight = 0.0
    if not disable_ph_loss and ph_weight_max > 0:
        ph_weight = linear_anneal(
            global_step, ent_anneal_steps, start=ph_weight_max, end=0.0
        )
        loss = loss + ph_weight * ph_loss

    # -----------------------------
    # consistancy loss
    # -----------------------------
    lambda_cons = 0.1
    cons_loss = aux["cons_loss"].mean()
    if epoch >= 3:
        loss += lambda_cons * cons_loss

    # -----------------------------
    # Entropy regularization (token attention)
    # -----------------------------
    lam_ent_w = 0.0
    if lam_ent > 0 and epoch >= entropy_start_epoch:
        lam_ent_w = linear_anneal(global_step, ent_anneal_steps, start=0.1 * lam_ent, end=lam_ent)

        tok_ent = aux["token_entropy"].mean()
        target_ent = float(aux["target_token_entropy"])

        delta = F.relu(tok_ent - target_ent)
        ent_unweighted = torch.clamp(delta, max=1.0) ** 2

        loss = loss + lam_ent_w * ent_unweighted
    
    # -----------------------------
    # Feature L2
    # -----------------------------
    feat_l2_term = lam_feat * feat.pow(2).sum(dim=1).mean()
    loss = loss + feat_l2_term


    # -----------------------------
    # Representation loss
    # -----------------------------
    representation_loss = torch.zeros((), device=logits.device)
    if c_map_mag is not None:
        representation_loss = c_map_mag.pow(2).mean()
        loss = accuracy_weight * loss + (1 - accuracy_weight) * representation_loss

    # -----------------------------
    # Metrics (use NON-mixup logits for interpretability)
    # -----------------------------
    with torch.no_grad():
        p_prob = torch.sigmoid(logits.view(-1))  # [B]
        correct = ((p_prob > 0.5).float() == y).sum().item()
        mse = ((y - p_prob) ** 2).mean().item()
        assert torch.isfinite(logits).all()

    loss_terms = {
        "bce_loss": float(bce_loss.detach().item()),
        "ph_loss": float(ph_loss.detach().item()),
        "ph_weight": float(ph_weight),
        "cons_loss": float(cons_loss.detach().item()),
        "lambda_cons": float(lambda_cons if epoch >= 3 else 0.0),
        "token_entropy_loss": float(ent_unweighted.detach().item()),
        "token_entropy_weight": float(lam_ent_w),
        "feature_l2": float(feat_l2_term.detach().item()),
        "representation_loss": float(representation_loss.detach().item()),
    }
...
def train_model(args, output):
    ...
    ph_weight_max = args.ph_weight_max
    disable_ph_loss = args.disable_ph_loss
    ...
    scalar_names  = ("score_scale", "log_tau_tok", "log_tau_ph", "pos_scale")
    ...
    tok_names = (
        ...
        "log_tau_ph",
        "score_scale",
        "pos_row", "pos_col", "pos_scale",
        "grid_post_gn", "grid_refine",
    ...
    log(f"\tlam_ent: {lam_ent}", file=output)
    log(f"\tentropy_start_epoch: {entropy_start_epoch}", file=output)
    log(f"\tph_weight_max: {ph_weight_max}", file=output)
    log(f"\tdisable_ph_loss: {disable_ph_loss}", file=output)
    ...
    ent_anneal_steps = steps_per_epoch * 2
    for epoch in range(num_epochs):
        ...
        for batch_idx, (z0, z1, y) in enumerate(train_iterator):
            ...
            loss, correct, mse, b, logits_used, loss_terms = interaction_grad(
                ...
                ph_weight_max=ph_weight_max,
                disable_ph_loss=disable_ph_loss,
                
            )
            #log(f"logit mean={logits_used.mean():.4f} std={logits_used.std():.4f}", file=output)
            #log(f"score_scale={model.clf.score_scale.item():.4f}  "
            #    f"tau={model.clf.log_tau_tok.exp().item():.4f}", file=output)
            
            loss.backward()

            #
            # if model.clf.score_scale.grad is not None:
            #     log(f"score_scale grad={model.clf.score_scale.grad.item():.6f}", file=output)
            # else:
            #     log("score_scale grad=None — BROKEN GRAD PATH", file=output)
            ...
            ph_accum += b * (loss_terms["ph_loss"] - ph_accum) / n
            cons_accum += b * (loss_terms["cons_loss"] - cons_accum) / n
            ent_accum += b * (loss_terms["token_entropy_loss"] - ent_accum) / n
            feat_l2_accum += b * (loss_terms["feature_l2"] - feat_l2_accum) / n
            rep_accum += b * (loss_terms["representation_loss"] - rep_accum) / n
            ...
            if report:
                ...
                tau_tok = float(model.clf.log_tau_tok.exp().item())
                score_scale = float(model.clf.score_scale.item())
                effective_gain = score_scale / max(tau_tok, 1e-8)
                log(
                    "train terms: "
                    f"bce={bce_accum:.6f} ph={ph_accum:.6f} cons={cons_accum:.6f} "
                    f"ent={ent_accum:.6f} feat_l2={feat_l2_accum:.6f} "
                    f"rep={rep_accum:.6f} ph_w={loss_terms['ph_weight']:.4f} "
                    f"ent_w={loss_terms['token_entropy_weight']:.4f} "
                    f"score_scale={score_scale:.4f} "
                    f"tau_tok={tau_tok:.4f} "
                    f"effective_gain={effective_gain:.4f} "
                    f"tau_ph={model.clf.log_tau_ph.exp().item():.4f}",
                    file=output,
                )
                    ...
                    run.log(
                        {
                            ...
                            "train/ph_loss": ph_accum,
                            "train/ph_weight": float(loss_terms["ph_weight"]),
                            "train/cons_loss": cons_accum,
                            "train/token_entropy_loss": ent_accum,
                            "train/token_entropy_weight": float(loss_terms["token_entropy_weight"]),
                            "train/feature_l2": feat_l2_accum,
                            "train/representation_loss": rep_accum,
                            "train/score_scale": score_scale,
                            "train/tau_tok": tau_tok,
                            "train/effective_gain": effective_gain,
                            "train/tau_ph": float(model.clf.log_tau_ph.exp().item()),
                        }
                    )
        ...
        log(f"Epoch {epoch+1}: lr={lr_now:.3e}", file=output)
        log(
            "Epoch train terms: "
            f"bce={bce_accum:.6f} ph={ph_accum:.6f} cons={cons_accum:.6f} "
            f"ent={ent_accum:.6f} feat_l2={feat_l2_accum:.6f} "
            f"rep={rep_accum:.6f} "
            f"score_scale={model.clf.score_scale.item():.4f} "
            f"tau_tok={model.clf.log_tau_tok.exp().item():.4f} "
            f"effective_gain={model.clf.score_scale.item() / max(model.clf.log_tau_tok.exp().item(), 1e-8):.4f} "
            f"tau_ph={model.clf.log_tau_ph.exp().item():.4f}",
            file=output,
        )

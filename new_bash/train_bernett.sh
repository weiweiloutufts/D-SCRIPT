...
TOPSY_TURVY=
TRAIN=/cluster/tufts/cowenlab/tt3d+/data/pair_files/bernett_train.tsv
TEST=/cluster/tufts/cowenlab/tt3d+/data/pair_files/bernett_test.tsv
...
OUTPUT_FOLDER=${OUTPUT_BASE}/bernett_esm2
OUTPUT_PREFIX=newtest
FOLDSEEK_FASTA=/cluster/tufts/cowenlab/wlou01/D-SCRIPT/data/???
...
usage() {
    ...
    -e: Embedding directory (default: $EMBEDDING_DIR)
...
EMBEDDING_DIR_FLAG="--embedding-dir ${EMBEDDING_DIR}"
...
dscript train \
    ...
    --save-prefix "${OUTPUT_FOLDER}/${OUTPUT_PREFIX}_ep_" \
    --device "${DEVICE}" \
    --lr 0.0005 \
    --lambda 0.05 \
    --num-epoch 10 \
